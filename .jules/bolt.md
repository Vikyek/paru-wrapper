...
## 2024-09-25 - Using `jq` to output shell variables
**Learning:** `jq` has a `@sh` filter that is extremely useful for safely formatting strings to be eval'd into Bash.
**Action:** Use `jq` with `@sh` when transforming JSON data into a format that needs to be consumed via `eval` into shell variables, especially for bulk assigning array elements instead of looping.
