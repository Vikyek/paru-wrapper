...
**Action:** Use a single `pacman -Sl` query to output a tabular list of *all* sync database packages (`repo name version`). This completely avoids lookup failures for missing packages, skips complex awk filtering, and executes extremely fast since it just reads the local DB cache without complex matching.
