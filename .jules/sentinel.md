...
## 2024-05-24 - Configuration File Generation Command Injection
**Vulnerability:** Found in `paru-wrapper` where unescaped user-provided arguments (`REPO_NAME`, `REPO_DB`, `PROJ_DIR`) were directly interpolated into double quotes within a `cat << EOF > file` block when generating a bash-sourced configuration file. This allows arbitrary command execution when the config is later sourced.
**Learning:** Shell here-documents (`cat << EOF`) resolve variables before writing to the file. If these variables are written inside double quotes (`VAR="$USER_INPUT"`) into a file that will later be `source`d by a shell script, any shell metacharacters like `"; command; #` are evaluated upon sourcing, leading to command injection.
**Prevention:** When writing variables to a file that will be evaluated as a shell script, always use Bash's safe quoting parameter expansion `${VAR@Q}` to automatically escape all shell metacharacters and safely format the variable as a quoted string, or format the file as JSON/INI and use a dedicated parser instead of `source`.
