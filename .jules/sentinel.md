...
## 2024-05-24 - Bash TOCTOU Symlink Overwrite
**Vulnerability:** Found a Time-of-Check to Time-of-Use (TOCTOU) symlink vulnerability in `paru-wrapper` where the script running as root writes cache states to a user-owned directory (`$CACHE_DIR`) using `echo` and `>`.
**Learning:** Redirecting output to a file in a user-controlled directory using standard shell redirection (`>`) as root is unsafe because a malicious user can replace the target file with a symlink pointing to a critical system file before the write occurs. The shell will blindly follow the symlink and overwrite the target file.
**Prevention:** If the script must write to a user's directory while running under `sudo`, explicitly drop privileges for the write operation (e.g., `sudo -u "$SUDO_USER" tee <file> > /dev/null`). If dropping privileges is not feasible, use `[ ! -L <file> ]` to assert the target is not a symlink before redirecting output to it.
