...
**Learning:** Hardcoding unsanitized input directly into a URL string opens the script to unintended argument injection/query parameter manipulation. Even though the vulnerability is simple, using standard string interpolation without escaping the input can introduce unintended logical bugs or severe injection flaws if an attacker manages to control the package name string. In addition, always be careful to limit your patch to exactly the fix so that the review system does not flag pre-existing syntax errors.
**Prevention:** Instead of string interpolation, use `curl -G` (to send a GET request) alongside `--data-urlencode "key=value"` to securely pass and automatically URL-encode dynamic query parameters.
## 2026-08-31 - URL Injection in Python API Requests
...
## 2024-05-24 - Command Injection via Config Generation in Shell Script
**Vulnerability:** Found a CRITICAL command injection vulnerability in `paru-wrapper` where user input (`$REPO_NAME`, `$REPO_DB`, etc.) was written to a configuration file inside double quotes: `PARU_WRAPPER_REPO="$REPO_NAME"`. By providing a repository name containing double quotes, semicolons, and comments (e.g. `hello"; echo PWNED; #`), an attacker could break out of the string assignment and inject arbitrary shell commands. When this generated configuration file was later `source`d by the script, the injected commands executed with the script's privileges.
**Learning:** Writing user-controlled input into shell scripts or configuration files that are later sourced (`source test.conf` or `. test.conf`) is highly dangerous if the input is not strictly escaped. Even when enclosed in double quotes, a user can break out of the string assignment if they can include double quote characters.
**Prevention:** When generating shell scripts or sourced config files that include dynamic variables, never use basic string interpolation or standard double quoting. Always safely escape the variables using `printf '%q'` (e.g., `PARU_WRAPPER_REPO=$(printf '%q' "$REPO_NAME")`) before writing them, which ensures all shell metacharacters are properly quoted and evaluated strictly as literal strings by bash.
