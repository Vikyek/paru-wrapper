...
## 2024-09-19 - Prevent Directory Symlink Overwrite
**Vulnerability:** TOCTOU arbitrary file overwrite and deletion via symlink attacks.
**Learning:** `mv -f` and `ln -sf` will move/link *into* the target directory if the target is a symlink pointing to a directory. Additionally, `rm -rf` will follow symlinks if the directory path has a trailing slash (e.g. from globbing `*/`), allowing attackers to delete arbitrary files.
**Prevention:** Always use `-T` (`--no-target-directory`) with `mv` and `ln` to treat the destination strictly as a file/symlink. Strip trailing slashes with `${d%/}` when passing globbed paths to `rm -rf`.
