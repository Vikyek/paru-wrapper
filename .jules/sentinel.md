...
## 2024-09-12 - Secure Atomic File Replacement
**Vulnerability:** TOCTOU symlink overwrite vulnerability when using `mv -f` without `-T`.
**Learning:** If the target of `mv -f` is a symlink pointing to a directory, `mv -f` will move the temporary file into the target directory rather than replacing the symlink itself, leaving the vulnerability open.
**Prevention:** Always use the `-T` (`--no-target-directory`) flag with `mv -f` (e.g., `mv -f -T "$tmp" "$target"`) to ensure the target is treated as a normal file and atomically replaced.
