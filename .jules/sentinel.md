...
## 2024-05-25 - TOCTOU and Symlink Dereferencing Vulnerabilities
**Vulnerability:** Found Time-of-Check to Time-of-Use (TOCTOU) symlink vulnerabilities when using `mv -f` to replace files and `ln -sf` to create symlinks in `pacman-wrapper`, `paru-wrapper`, and `paru-wrapper.install`. Additionally, `rm -rf` was called on globbed directories with a trailing slash (`$d`), which dereferences symlinks and deletes their target's contents instead of the symlink itself.
**Learning:** Without the `-T` flag, if the target of `mv -f` or `ln -sf` is a symlink pointing to an existing directory, the command will unexpectedly move the source file or nest the symlink inside that target directory instead of replacing the symlink itself. Similarly, passing a path with a trailing slash to `rm -rf` causes it to dereference the symlink.
**Prevention:** Always use the `-T` (`--no-target-directory`) flag with `mv` and `ln` when explicitly replacing a file or symlink to ensure the target is treated as a normal file. Always strip trailing slashes (e.g. `${d%/}`) when passing dynamic directory paths to `rm -rf`.
