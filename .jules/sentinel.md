...
## 2026-08-31 - URL Injection in Bash curl commands
**Vulnerability:** Found a URL injection vulnerability in `paru-wrapper-gittinator-impl` where dynamic package names were inserted into an AUR RPC URL via string interpolation (`URL_PARAMS+="&arg[]=${candidate}"` -> `curl -s "https://aur.archlinux.org/rpc/v5/info?${URL_PARAMS}"`).
**Learning:** Manual string interpolation for query parameters fails to URL-encode special characters in bash scripts as well. In Arch Linux, packages can contain characters like `+` (e.g., `gcc++`). If unencoded, the `+` is interpreted as a space by the server, causing logical bugs (e.g., retrieving `gcc  ` instead of `gcc++`) and allowing potential parameter injection.
**Prevention:** Always use `curl -G` alongside `--data-urlencode "key=value"` to securely pass and automatically URL-encode dynamic query parameters in bash scripts.
