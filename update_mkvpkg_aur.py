...
def parse_evr(evr):
    ...
    s = evr
    epoch_idx = 0
    while epoch_idx < len(s) and s[epoch_idx].isdigit():
        epoch_idx += 1

    if epoch_idx < len(s) and s[epoch_idx] == ':':
        epoch = evr[:epoch_idx]
        if not epoch:
            epoch = "0"
        version = evr[epoch_idx+1:]
    else:
        epoch = "0"
        version = evr

    # Check for release
    rel_idx = version.rfind('-')
    if rel_idx != -1:
        release = version[rel_idx+1:]
        version = version[:rel_idx]
...
def rpmvercmp(a, b):
    ...
    while ptr1 < len(a) and ptr2 < len(b):
        while ptr1 < len(a) and not a[ptr1].isalnum():
            ptr1 += 1
        while ptr2 < len(b) and not b[ptr2].isalnum():
            ptr2 += 1
        ...
        if a[ptr1].isdigit():
            while ptr1 < len(a) and a[ptr1].isdigit():
                ptr1 += 1
            while ptr2 < len(b) and b[ptr2].isdigit():
                ptr2 += 1
            isnum = True
        else:
            while ptr1 < len(a) and a[ptr1].isalpha():
                ptr1 += 1
            while ptr2 < len(b) and b[ptr2].isalpha():
                ptr2 += 1
            isnum = False
        ...
        if isnum:
            seg1_val = seg1.lstrip('0')
            seg2_val = seg2.lstrip('0')

            if len(seg1_val) > len(seg2_val): return 1
            if len(seg2_val) > len(seg1_val): return -1

            if seg1_val < seg2_val: return -1
            if seg1_val > seg2_val: return 1
        else:
            if seg1 < seg2: return -1
            if seg1 > seg2: return 1

    if ptr1 >= len(a) and ptr2 >= len(b):
        return 0

    if ptr1 >= len(a):
        if ptr2 < len(b) and not b[ptr2].isalpha():
            return -1
        elif ptr2 < len(b) and b[ptr2].isalpha():
            return 1
        else:
            return -1
    else:
...
def main():
    ...
    # Optimization: Use pure python port of alpm_vercmp to avoid subprocess overhead
    for pkg in unmodified_pkgs:
